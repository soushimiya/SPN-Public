...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		for (file in FileSystem.readDirectory("assets/videos")) {
			if (file.endsWith('.mp4')) {
				var video:VideoHandler = new VideoHandler();
				video.play(Paths.video(file));
				video.volume = 0;
	...
	function startIntro()
	{
		...
		ngSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('fuckyou'));
		add(ngSpr);
		ngSpr.visible = false;
		ngSpr.setGraphicSize(Std.int(ngSpr.width * 0.65));
		ngSpr.updateHitbox();
	...
	override function beatHit()
	{
		...
		FlxTween.tween(FlxG.camera, {zoom: 1}, Conductor.bpm / 240 + 0.05, {ease:FlxEase.circOut});
		if(logoBl != null)
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 2:
					createCoolText(['SPN Team']);
				case 4:
					addMoreText('PRESENTS');
				case 5:
					deleteCoolText();
				case 6:
					createCoolText(['Soni is Not', "Sonic's Personification"], -40);
				case 8:
					addMoreText('Go Fuck Yourself', -40);
					ngSpr.visible = true;
				case 9:
					deleteCoolText();
					ngSpr.visible = false;
